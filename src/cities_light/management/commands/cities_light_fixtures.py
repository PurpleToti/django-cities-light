...
going to take more time):
    ...
    def add_arguments(self, parser):
        parser.add_argument("subcommand", type=str, help="Subcommand (load/dump)")
        parser.add_argument(
            "--natural-foreign",
            action="store_true",
            default=False,
    ...
    def handle(self, *args, **options):
        ...
        self.country_path = os.path.join(fixtures_dir, self.COUNTRY_FIXTURE)
        self.region_path = os.path.join(fixtures_dir, self.REGION_FIXTURE)
        self.subregion_path = os.path.join(fixtures_dir, self.SUBREGION_FIXTURE)
        self.city_path = os.path.join(fixtures_dir, self.CITY_FIXTURE)

        subcommand = options.get("subcommand")

        if subcommand not in ("load", "dump"):
            raise CommandError('Please specify either "load" or "dump" command')

        if subcommand == "load":
            base_url = options.get("base_url") or FIXTURES_BASE_URL
            if base_url is None:
                raise CommandError(
                    "Please specify --base-url or "
                    "settings.CITIES_LIGHT_FIXTURES_BASE_URL"
                )
    ...
    def dump_fixtures(self):
        ...
        self.dump_fixture("{}.Country".format(CITIES_LIGHT_APP_NAME), self.country_path)
        self.dump_fixture("{}.Region".format(CITIES_LIGHT_APP_NAME), self.region_path)
        self.dump_fixture(
            "{}.SubRegion".format(CITIES_LIGHT_APP_NAME), self.subregion_path
        )
        self.dump_fixture("{}.City".format(CITIES_LIGHT_APP_NAME), self.city_path)
    ...
    def load_fixture(self, source, destination, force=False):
        ...
        downloader.download(source=source, destination=destination, force=force)
