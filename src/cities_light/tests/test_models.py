...
class TestModels(test.TransactionTestCase):
    ...
    def test_timezone_validator(self):
        ...
        self.assertEqual(e.exception.messages[0], "Timezone validation error: None")
        self.assertEqual(e.exception.code, "timezone_error")

        with self.assertRaises(ValidationError) as e:
            timezone_validator("")
        self.assertEqual(e.exception.messages[0], "Timezone validation error: ")
        self.assertEqual(e.exception.code, "timezone_error")
        ...
        try:
            ...
            self.assertTrue(
                False, "Error call timezone_validator with true value: %s" % value
            )
    ...
    def test_city_search_names_icontains(self):
        ...
        country = country_model(
            name="Country", name_ascii="Country", geoname_id="123456", continent="EU"
        )
        country.save()
        ...
        city_qs = city_model.objects.filter(search_names__icontains="First City")

        self.assertEqual(len(city_qs), 1, msg="Should find 1 city")
        self.assertEqual(city_qs[0].name, city1.name)

        city_qs = city_model.objects.filter(search_names__icontains="Second City")

        self.assertEqual(len(city_qs), 1, msg="Should find 1 city")
        self.assertEqual(city_qs[0].name, city2.name)

        city_qs = city_model.objects.filter(search_names__icontains="Third City")
        self.assertEqual(len(city_qs), 0, msg="Should find 0 cities")

        city_qs = city_model.objects.filter(search_names__icontains="Region Country")
        self.assertEqual(len(city_qs), 2, msg="Should find 2 cities")
        self.assertEqual(city_qs[0].name, city1.name)
        self.assertEqual(city_qs[1].name, city2.name)

        city_qs = city_model.objects.filter(search_names__icontains="gion coun")
        self.assertEqual(len(city_qs), 2, msg="Should find 2 cities")
        self.assertEqual(city_qs[0].name, city1.name)
        self.assertEqual(city_qs[1].name, city2.name)
